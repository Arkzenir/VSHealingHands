...
public class VSHealingHandsModSystem : ModSystem
{
    ...
    bool TryInjectInterceptBehavior(CollectibleObject? col)
    {
        ...
        // 1.21.5: the healing behavior class is BehaviorHealingItem, and Health lives
        // under its Config (a HealOverTimeConfig), not as a direct property like 1.22+.
        BehaviorHealingItem? healBehavior = col.GetCollectibleBehavior<BehaviorHealingItem>(withInheritance: true);
        ...
        // BehaviorHealingItem's Start sets EnumHandling.PreventSubsequent (which would
        // otherwise stop the loop before we run), and so our Stop runs before
        // vanilla's Stop calls ReceiveDamage.
