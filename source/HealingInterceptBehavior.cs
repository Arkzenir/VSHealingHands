...
/// <summary>
/// Prepended (index 0) onto every item/block whose BehaviorHealingItem actually
/// restores health. Realizes all three healing modifiers without ever mutating that
/// shared behavior's own fields - unlike a mutate-then-revert-on-the-shared-instance
/// approach, nothing here is written by more than one logical actor, so correctness
/// does not depend on the server's threading model.
/// <br/><br/>
/// <b>Why prepend.</b> BehaviorHealingItem.OnHeldInteractStart sets
...
/// BehaviorHealingItem.GetApplicationTime, which reads
...
public sealed class HealingInterceptBehavior : CollectibleBehavior
{
    ...
    bool GetAffectedByArmor()
    {
        ...
        // 1.21.5: the item's own config lives under BehaviorHealingItem.Config
        // (a HealOverTimeConfig), not as direct properties like 1.22+.
        BehaviorHealingItem? healBehavior = collObj.GetCollectibleBehavior<BehaviorHealingItem>(withInheritance: true);
        affectedByArmor = healBehavior?.Config.AffectedByArmor ?? true;
        return affectedByArmor.Value;
    }

    /// <summary>
    /// Resolves the heal target the way BehaviorHealingItem.GetTargetEntity does: the
    /// aimed-at entity when the healer holds Ctrl, is stationary, and that entity is
    /// healable; otherwise the healer themselves. Kept in sync with vanilla so our
    /// resolved target always matches the entity vanilla actually heals.
    /// </summary>
